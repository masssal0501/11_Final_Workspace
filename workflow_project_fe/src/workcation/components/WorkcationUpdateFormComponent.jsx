...
import "../styles/WorkcationUpdate.css";
            ...
            getHubList({
                ...
                .then(res => {
                    ...
                    setOptionPlaceData(prev => {
                        if (!prev[key] || !prev[key].item) return prev;
                        const currentHubNo = prev[key].item.hubNo;
                        const matchedItem = listData.find(item => String(item.hubNo) === String(currentHubNo));
                        if (matchedItem) {
                            return {
                                ...prev,
                                [key]: { ...prev[key], item: matchedItem }
                            }
                        }
                        return prev;
                    })
    ...
    useEffect(() => {
        ...
        getWorkcationDetail(workcationNo)
            .then(res => {
                ...
                setHubType(res.placeType || "office");
                setSelectHubNo(res.hubNo ? String(res.hubNo) : "");
                setTransportText(res.transportText !== undefined ? res.transportText : "");
                setEtcText(res.etcText !== undefined ? res.etcText : "");
                setLocalGovSupport(res.localGovSupport || 0);

                if (res.planList) {
                    setPlanList(res.planList.map((p, idx) => ({
                ...
                if (res.option && Array.isArray(res.option)) {
                    const loadedOption = {};
                    const loadedBtns = [];
                    res.option.forEach(option => {
                        loadedBtns.push(option.type);
                        loadedOption[option.type] = {
                            item: {
                                hubNo: option.hubNo,
                                price: option.price || 0,
                                hubName: option.hubName || ""
                            },
                            date: option.visitDate || getTomorrow()
                        }
                    })
                    setSelectedBtns(loadedBtns);
                    setOptionPlaceData(loadedOption);
                }
            })
        ...
        <div className="workcatrion-enroll-container">
            ...
            <div className="common-btn-group">
                ...
                <button
                    ...
                    onClick={() => navigate(`/workcation/detail/${workcationNo}`)}>
                    취소
                </button>
            </div>
            ...
            <div className="all-price-container">
                ...
                <div className="left-price-box">
                    ...
                    <div className="left-price-row">
                        ...
                        <div className="left-price-personal">
                            <span>예상 지출액</span>
