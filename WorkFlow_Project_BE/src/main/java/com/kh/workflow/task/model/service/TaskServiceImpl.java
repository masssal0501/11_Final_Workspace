 ...
 // 상세
    ...
    public Map<String, Object> selectTaskDetail(Integer taskNo) {
        ...
        List<TaskHistory> historyList =
                taskHistoryDao.findByTaskTaskNoOrderByCreatedAtDesc(taskNo);
    ...
    @Override
    @Transactional(readOnly = true)
    public Page<Map<String, Object>> selectTaskBoardList(
            String keyword,
            Pageable pageable) {
        ...
        Page<WorkcationInfo> page =
                workcationDao.findTaskBoardList(
                        searchKeyword,
                        pageable
                );
        ...
        return page.map(workcation -> {
            ...
            List<Work> workList =
                    workDao.findByWorkcationWorkcationNo(
            ...
            for (Work work : workList) {
                List<Task> tasks =
                        taskDao.findByWorkWorkNo(
                                work.getWorkNo()
                        );

                for (Task task : tasks) {
                    taskCount++;
                    progressSum += task.getProgress() != null
                            ? task.getProgress()
                            : 0;
                }
            }
            ...
            int overallProgress =
                    taskCount > 0
                            ? Math.round((float) progressSum / taskCount)
                            : 0;
            ...
            map.put(
                    ...
                    workcation.getEmployee() != null
                            ? workcation.getEmployee().getEmpName()
                            : "-"
            );
    ...
    @Override
    @Transactional(readOnly = true)
    public Map<String, Object> selectWorkcationTasks(
            Integer workcationNo) {

        WorkcationInfo workcation =
                workcationDao.findById(workcationNo)
                        .orElseThrow(() ->
        ...
        List<Work> workList =
                workDao.findByWorkcationWorkcationNo(workcationNo);
        ...
        for (Work work : workList) {
            ...
            List<Task> tasks =
                    taskDao.findByWorkWorkNo(work.getWorkNo());
        ...
        int overallProgress =
                taskList.size() > 0
                        ? Math.round(
                                (float) progressSum / taskList.size()
                        )
                        : 0;
